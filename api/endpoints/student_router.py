...
async def update_student(db:AsyncIOMotorClient = Depends(get_database), student:student_model.UpdateStudentModel = Body(...), student_name: str = ""):
...
async def delete_student(db: AsyncIOMotorClient = Depends(get_database), student_name: str = ""):
    await get_db_student(db, student_name) 
